...
interface AddMembersDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onAddMembers: (members: Member[], callType: 'voice' | 'video') => void;
  ...
  const handleAddMembers = () => {
    if (selectedMembers.length === 0) {
      toast({
        ...
        description: 'Please select at least one member to add to the call.',
        variant: 'default',
      });
      return;
    }

    onAddMembers(selectedMembers, callType);
    onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Add Members to {callType.charAt(0).toUpperCase() + callType.slice(1)} Call</DialogTitle>
          <DialogDescription>
            Select members to invite to the current call.
          </DialogDescription>
        </DialogHeader>
